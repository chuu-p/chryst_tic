...
, libdecor, pipewire, libpulseaudio, libiconv, libxcb, withPro ? false, ... }:
with pkgs;
let
  major = "1";
  minor = "1";
...
in stdenv.mkDerivation rec {
  ...
  postUnpack = ''
    ...
    sed -i 's/pw_node_enum_params(node->proxy,/pw_node_enum_params((struct pw_node *)node->proxy,/' TIC-80/vendor/sdl2/src/audio/pipewire/SDL_pipewire.c
  '';

  cmakeFlags = (if withPro then [ "-DBUILD_PRO=On" ] else [])
               ++ ["-DBUILD_SDLGPU=On" "-DCMAKE_POLICY_VERSION_MINIMUM=3.5"];
  enableParallelBuilding = true;
  dontStrip = true;
  buildInputs = [ cmake pkg-config wayland-protocols python3 curl ] ++ dlopenBuildInputs;
  dlopenBuildInputs = [
    libGL
    libGLU
    libxcb
